...
nvim.ex.set("nowrap")
do
  local options = {encoding = "utf-8", spelllang = "en_us", backspace = "2", colorcolumn = "80", errorbells = true, number = true, ruler = true, completeopt = "menuone,noselect", wildmenu = true, wildignore = "*/tmp/*,*.so,*.swp,*.zip", ignorecase = true, smartcase = true, clipboard = "unnamedplus", list = true, listchars = str.join(",", {"tab:\226\150\182-", "trail:\226\128\162", "extends:\194\187", "precedes:\194\171", "eol:\194\172"}), expandtab = true, tabstop = 4, shiftwidth = 4, softtabstop = 4, undofile = true, splitbelow = true, splitright = true, hlsearch = true, statusline = str.join(" ", {"[%n]", "%f", "%m%y%r%h%w%=%-35.(%{&fenc==\"\"?&enc:&fenc}", "[%{&ff}]", "[%L,%p%%]", "[%l,%c%V]", "%)%P"}), signcolumn = "number", relativenumber = true, backup = false, showmode = false, swapfile = false}
