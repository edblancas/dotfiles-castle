...
conditions = {["buffer-not-empty"] = _3_, ["hide-in-width"] = _4_, ["check-git-workspace"] = _5_}
...
local function _8_()
  local mode_color = {n = colors.red, i = colors.green, v = colors.blue, V = colors.blue, c = colors.magenta, no = colors.red, s = colors.orange, S = colors.orange, ic = colors.yellow, R = colors.violet, Rv = colors.violet, cv = colors.red, ce = colors.red, r = colors.cyan, rm = colors.cyan, ["r?"] = colors.cyan, ["!"] = colors.red, t = colors.red}
  return {fg = mode_color[vim.fn.mode()]}
end
ins_left({_7_, color = _8_, padding = {right = 1}})
ins_left({component = "filesize", cond = conditions["buffer-not-empty"]})
ins_left({component = "filename", cond = conditions["buffer-not-empty"], color = {fg = colors.magenta, gui = "bold"}})
ins_left({component = "location"})
ins_left({component = "progress", color = {fg = colors.fg, gui = "bold"}})
ins_left({component = "diagnostics", sources = {"nvim_diagnostic"}, symbols = {error = "\239\129\151 ", warn = "\239\129\177 ", info = "\239\129\170 "}, ["diagnostics-color"] = {error = {fg = colors.red}, warn = {fg = colors.yellow}, info = {fg = colors.cyan}}})
local function _9_()
  return "%="
end
ins_left({["1"] = _9_})
...
ins_right({component = "o:encoding", fmt = string.upper, cond = conditions["hide-in-width"], color = {fg = colors.green, gui = "bold"}})
ins_right({component = "fileformat", fmt = string.upper, color = {fg = colors.green, gui = "bold"}, icons_enabled = false})
ins_right({component = "branch", icon = "\239\145\191", color = {fg = colors.violet, gui = "bold"}})
ins_right({component = "diff", symbols = {added = "\239\131\190 ", modified = "\243\176\157\164 ", removed = "\239\133\134 "}, diff_color = {added = {fg = colors.green}, modified = {fg = colors.orange}, removed = {fg = colors.red}}, cond = conditions["hide-in-width"]})
local function _13_()
  return "\226\150\138"
end
